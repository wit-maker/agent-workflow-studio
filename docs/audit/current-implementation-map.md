...
| Execution and recovery | `runPlanner.ts`, `runEngine.ts`, `nodeExecutors.ts`, `executionGraph.ts`, `connectorQueue.ts`, `recoveryActions.ts`, `runTrace.ts`, `runAudit.ts`, `runDetail.ts` | Provides local mock run planning, execution summaries, graph state, connector jobs, retry/review/skip/cancel actions, current run trace evidence, durable safe audit snapshot creation/revival, read-only run replay view models, and safe audit comparison view models with metadata, step evidence grouping, and focused node/edge scope. |
...
| `WorkflowConnection` | Has source/target node and port ids, `ConnectionKind`, carried data types, status, metrics, and optional safe `runtimePolicy` metadata for condition / delay / retry / error-route summaries. Runtime edge HUD derives observed route/health from execution graph and run trace, and can also display this optional policy. | Does not yet persist edge-level audit events or enforce condition/retry semantics as first-class runtime contracts. |
...
| Execution graph | Review/error/retry/skip routes are summarized and projected into selected-edge HUD / React Flow runtime edge classes. Optional connection runtime policy metadata is shown as safe summaries but is not enforced. |
