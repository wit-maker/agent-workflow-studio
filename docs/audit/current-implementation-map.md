...
| Run Detail Replay / Diff | `RunDetailPanel` comparison includes runtime event count metadata, step evidence grouping, focused node/edge scoped diff, focused edge safe route-event metadata rows when two safe audit snapshots are selected, a replay-ready safe runtime timeline for the selected current trace or revived audit snapshot, and a metadata-only replay candidate with frame navigation and session-only Play/Pause. Revived `RunTrace` values now carry normalized safe `traceAudit.runtimeEvents`, so timeline/replay paths use the same safe event source for current and history snapshots. | This is safe event playback over metadata, not full route reconstruction, visual run animation, or expression-backed replay. |
...
| Direct QA Harness | `npm.cmd run qa:direct` exercises import/export validation, safe audit/run history summaries, two-run Run Detail comparison, current-trace and revived audit timeline/replay focus, selected Edge HUD safe copy, HUD notification session read/ack/pin projection, legacy audit normalization, and storage-key registry checks without Browser file picker/download support. | This supplements Browser QA; it does not replace rendered interaction QA. |
...
| Bottom Console `注意信号` tab + `HudSignalList` component | Supplementary read-only signal inspection list derived from `HudSnapshot`. | Not the Cognitive HUD body. The Cognitive HUD is expressed through the canvas/shell attention layer: node badges, edge highlight/dimming, focus overlay, critical card, minimal HUD strip, and selected target emphasis. |
...
| BottomMonitor `実行詳細` tab + `RunDetailPanel` | Read-only Run Trace step evidence viewer that can select current runtime trace or revived safe audit snapshots from run history, with node/edge focus deep links back to the canvas. It also has a replay-ready safe runtime timeline, a metadata-only replay candidate with frame navigation, a two-run safe audit comparison view with metadata rows, runtime event counts, step-level safe evidence grouping, focused node/edge scoped diff, focused edge safe route-event metadata diff, selected-edge durable replay evidence, and safe connection `runtimePolicy` summaries inside the same panel. | Not a full animated replay engine, not full visual route reconstruction, not visual route animation, and not expression evaluation. |
...
| Overlay | `CognitiveHudOverlay` shows central HUD variants for semantic attention, including compact validation-warning and review-required state cues when those semantic focus paths are active. `HudNotificationBundle` shows on-demand HUD feed, run history summary, density settings, session-only read/ack/pin controls, and Replay links into Run Detail. `SelectedObjectHud` shows selected-node local HUD context and node-scoped durable audit evidence near the selected node while avoiding measured HUD surfaces. `SelectedEdgeHud` shows selected-edge flow context plus runtime route, optional connection runtime policy, source/target step status, evidence count, trace source, health, durable safe replay summary, next action, and an Open Run Detail focus link near the selected flow. `CriticalOverlay` foregrounds failure/danger variants when semantic or HUD priority is critical. | Durable persisted notification state, animated audit replay, visual route reconstruction, and critical short-tone audio are not implemented. |
...
| Execution graph | Review/error/retry/skip routes and safe runtime events are summarized and projected into selected-edge HUD / React Flow runtime edge classes / Run Detail metadata diff / Run Detail replay-ready timeline / Run Detail metadata-only replay candidate for both current traces and revived audit snapshots. Fixed preset connection runtime policies can now gate mock routes as `main` or `skip`; expression metadata is still displayed but not evaluated. |
| Evaluation | Local evaluation scores and rebuild requests are visible. |
| Missing | Full animated replay engine, full edge runtime policy enforcement, full edge-level visual route reconstruction, node log history, visual multi-run replay animation, error rate, parallelism, and resource load. Safe `traceAudit` snapshot selection/recall, metadata comparison, metadata-only replay candidate, replay-ready event ordering, step evidence grouping, and focused scoped diff exist. |
...
| HudSnapshot / HudCounts | Implemented as `HudSnapshot` / `HudCounts` | `HudSignalList` plus canvas/shell HUD projections |
| Priority Score / L0–L5 alert level | Partial — alert level + priority derived per signal | `HudSignalList` plus canvas/shell HUD projections |
...
| Cognitive HUD as attention-allocation layer spanning Canvas / Inspector / modals | Partial — canvas selection/focus layer, node badges, edge highlight/dimming/replay warning classes, focus overlay, critical card, minimal HUD strip, selected target emphasis, and on-demand notification/history/density overlay exist, but durable notification state and cross-modal intervention policy are incomplete | `HudSignalList` is only a supplementary signal inspection surface |
...
| Incident Replay | Partial — Run Detail has replay-ready safe runtime timeline plus a metadata-only replay candidate with frame navigation and session-only Play/Pause. It is not a visual route reconstruction engine. | `RunDetailPanel` |
