...
- `SelectedEdgeHud` and React Flow edges now derive delay / retry / error-route / health from current `ExecutionGraph`, `RunTrace`, `WorkflowConnection`, validation results, optional connection `runtimePolicy` metadata, and a mock-only fixed-preset policy evaluator. Mock run can gate routes for fixed presets as `main` or `skip`, Run Detail can step through safe metadata replay frames, and edge selection is lifted to `AppShell` for Run Detail deep links. Remaining debt is full branch graph enforcement, durable route replay, full visual replay animation, and edge-level audit records. Arbitrary expression evaluation remains intentionally absent.
