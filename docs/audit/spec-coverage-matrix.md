...
|---|---|---|---|
| UI-01 Main canvas | Partial | React Flow is now the default fresh canvas and shows 12 nodes, 13 edges, status, ports, validation, inline previews, minimap HUD, zoom-mode classes, workflow group layer, measured anchored canvas-overlay HUDs, and selected path dimming/highlight. Standard canvas remains as fallback. | Full editing, auto layout, DnD node creation, complete node deletion UX, semantic focus path, advanced HUD density/reflow rules, and complete React Flow migration are incomplete. |
...
| UI-11 Cognitive HUD settings | Missing | No dedicated HUD settings screen. The cognitive HUD now has receiver surfaces — `CognitiveHudOverlay` (canvas), anchored `SelectedObjectHud` (selected-node canvas overlay), anchored `SelectedEdgeHud` (selected-edge canvas overlay), right panel `Situation` mode, `CanvasCommandHud`, minimap HUD, selected path dimming/highlight, and `CriticalOverlay` (root) — plus node-level HUD badges for failed/review/blocked/retry. The Detail Drawer `認知HUD` tab is a summary view of derived signals, not the HUD layer itself. | HUD priority settings, danger visibility settings, notification settings, depth/collapse rules, always-on edge badges, central HUD card with rich content, approval-pending HUD, failure-cause card, semantic focus overlay, notification bundle, critical short audio cue, and HUD history are not modeled. |
| UI-13 Cognitive Workspace Shell (Issue #34) | Partial | `GameHudShell` now turns the workspace into a Canvas First layout. Palette, detail, and BottomMonitor surfaces are HUD drawer/console overlays. `CanvasCommandHud` provides minimal always-on controls. Selected nodes and edges show measured anchored floating HUDs that avoid DOM-measured HUD surfaces. MiniMap, zoom-mode representation, inline previews, static workflow groups, and selected node/edge focus dimming/highlight are implemented. | Semantic focus from runtime state, full per-region content (Workflow Library, Templates), removal of duplicate BottomMonitor tabs, rich central HUD variants, advanced HUD density/reflow policy, and assistant non-text channels are not implemented. |
...
| Cognitive HUD | Partial | `HudSignal` / `HudSnapshot` / `HudCounts` derive priority, summary, focus, zoom HUD, inline preview, workflow group, selected-node HUD, and selected-edge HUD projections. Node status badges, measured selected-node object HUD, measured selected-edge flow HUD, selected path dimming/highlight, minimap HUD, zoom mode, inline previews, and bottleneck indicators are visible. | Cognitive HUD as a complete **attention-allocation layer** (selection / compression / emphasis / notification / intervention) spanning Canvas / Inspector / modals is still incomplete. Always-on edge badges, rich central HUD variants, approval-pending HUD, failure-cause card, semantic focus overlay, notification bundle, critical short audio cue, HUD history, and the 5 design dimensions (what / when / where / how / how-much to show) are not modeled. |
