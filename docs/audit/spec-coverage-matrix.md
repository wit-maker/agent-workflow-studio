...
|---|---|---|---|
| UI-01 Main canvas | Partial | React Flow is now the default fresh canvas and shows 12 nodes, 13 edges, status, ports, validation, inline previews, minimap HUD, zoom-mode classes, workflow group layer, and canvas-overlay HUDs. Standard canvas remains as fallback. | Full editing, auto layout, DnD node creation, complete node deletion UX, coordinate-following HUD, path dimming, and complete React Flow migration are incomplete. |
