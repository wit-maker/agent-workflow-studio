...
|---|---|---|---|---|---|
| UI-01 全体俯瞰 / メインキャンバス | Workflow全体をノード、接続線、状態、実行操作、検証結果として俯瞰できる。Runで状態変化とログ追加が見える。 | `src/components/AppShell.tsx`, `src/components/WorkflowCanvas.tsx`, `src/components/ReactFlowCanvas.tsx`, `src/components/ReactFlowNode.tsx`, `src/components/NodeCard.tsx`, `src/components/ConnectionLine.tsx`, `src/components/ConnectionEditor.tsx`, `src/domain/reactFlowAdapter.ts`, `src/domain/sampleWorkflow.ts` | Phase 7 までで部分実装済み。12個のMVPノード、接続線、Runモック、接続検証、選択式接続作成/削除に加え、標準Canvasと React Flow Canvas の切替、Port Handle 表示、ドラッグ接続、Edge 選択削除、Inspector 連動がある。 | ミニマップ、自動レイアウト、ノード追加DnD、高度なEdge編集、ノード位置の永続保存は未実装。 | Phase 8 で React Flow 側の位置保存、ノード追加、レイアウト、拡張編集を段階的に検討する。 |
