...
|---|---|---|---|---|---|
| UI-01 全体俯瞰 / メインキャンバス | Workflow全体をノード、接続線、状態、実行操作、検証結果として俯瞰できる。Runで状態変化とログ追加が見える。 | `src/components/AppShell.tsx`, `src/components/WorkflowCanvas.tsx`, `src/components/ReactFlowCanvas.tsx`, `src/components/ReactFlowNode.tsx`, `src/components/NodeCard.tsx`, `src/components/ConnectionLine.tsx`, `src/components/ConnectionEditor.tsx`, `src/domain/reactFlowAdapter.ts`, `src/domain/sampleWorkflow.ts`, `src/storage/localCanvasState.ts` | Phase 7.1 までで部分実装済み。12個のMVPノード、接続線、Runモック、接続検証、選択式接続作成/削除に加え、標準Canvasと React Flow Canvas の切替、Canvas 表示モード保存、Port Handle 表示、ドラッグ接続、Edge 選択削除、Inspector 連動、React Flow ノード位置保存、位置リセットがある。 | ミニマップ、自動レイアウト、ノード追加DnD、高度なEdge編集、本格 Undo / Redo、React Flow 全面移行は未実装。 | Phase 7.2 で React Flow Canvas の操作性改善を進め、その後にノード追加、レイアウト、拡張編集を段階的に検討する。 |
