...
export function buildBriefingPrompt(input: BriefingInput): string {
  ...
  return [
    ...
    `- events: ${input.runtimeReplay.eventCount}`,
    `- routeEvents: ${input.runtimeReplay.routeEventCount}`,
    `- edgeEvents: ${input.runtimeReplay.edgeEventCount}`,
    `- reviewEvents: ${input.runtimeReplay.reviewEventCount}`,
    `- warn: ${input.runtimeReplay.warningEventCount} / error: ${input.runtimeReplay.errorEventCount} / retry: ${input.runtimeReplay.retryEventCount}`,
    `- flowPressure: ${input.runtimeReplay.flowPressureLabel} / ${input.runtimeReplay.flowPressureLevel}`,
    `- flowSummary: ${input.runtimeReplay.flowPressureSummary}`,
    `- templateHistoryHint: ${input.runtimeReplay.templateHistoryHint}`,
    `- latest: ${input.runtimeReplay.latestSummary ?? 'none'}`,
    `- hint: ${input.runtimeReplay.replayHint}`,
    '',
    '# Safety Warnings',
    ...
    '',
    '## Connectors',
    `- rail: ${input.connectors.railSummary}`,
    `- invalidConnections: ${input.connectors.invalidConnections}`,
    `- retryReady: ${input.connectors.retryReady}`,
    `- rateLimitPlaceholder: ${input.connectors.rateLimitPlaceholder}`,
